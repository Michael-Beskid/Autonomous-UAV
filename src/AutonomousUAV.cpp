...
void getDesStateManual() {
  ...
  thro_des = (radio.getPWM(3) - 1000.0)/1000.0; //Between 0 and 1
  roll_des = (radio.getPWM(1) - 1500.0)/500.0; //Between -1 and 1
  pitch_des = (radio.getPWM(2) - 1500.0)/500.0; //Between -1 and 1
...
void controlANGLE() {
  ...
  if (radio.getPWM(3) < 1060) {   //Don't let integrator build if throttle is too low
    integral_roll = 0;
  }
